...
class Environment(gym.Env): 
    ...
    def __init__(self,  skills_goal, param_dict, reduced = False, final_goal = np.array([16.,16.,16.,16.,16.,16.]) ):
        ...
        if self.reduced:
            self.final_goal =  np.array([2.,2.,2.,2.,2.,2.])
        ...
        state_shape = len(self.question_recycler.get_state_features(0,self.skills_goal))
        ...
        for name in self.task_names:
            self.task_cost_distributions.append(lognorm(s = param_dict["games_times"][name]["shape"], loc = param_dict["games_times"][name]["loc"], scale = param_dict["games_times"][name]["scale"]))

        f = {}
        for i in range(self.n_tasks):
            f[self.task_names[i]] = self.task_cost_distributions[i].mean()[0]/1000 * -1
        print(f)
        ...
        self.success_probabilities = param_dict["Success Probabilities"]
    ...
    def step(self, action):
        ...
        self.question_recycler.update_qr_levels_and_delays(selected_questions)
        
        # for the small MDP, delay = 1 after every game
        if self.reduced:
            self.question_recycler.update_delays_for_new_session(1)

        # for the large MDP, first it is determined whether the action is the last of the day and if that is the case, a delay is drawn from the delay distribution 
        else:
            p_last_game_of_the_day = self.games_per_day[self.n_games_current_day]
            
            r = np.random.uniform(size = 1)
            
            # new day, delays are updated 
            if r < p_last_game_of_the_day:
                self.question_recycler.update_delays_for_new_session(self.delay_between_game_days.rvs(1))
                info["games in day"] = self.n_games_current_day
                self.n_games_current_day = 1
            # day continues    
            else:
                self.n_games_current_day += 1 
            ...
            self.question_recycler.update_session_state(self.games_per_day[self.n_games_current_day])
    ...
    def get_action_cost(self, action):
        ...
        return self.task_cost_distributions[action].rvs(1) * -0.01
    ...
    def find_next_states_a(self, initial_state, action, var_delay = False):
        ...
        for answer in answers:
            ...
            self.question_recycler.update_qr_levels_and_delays(temp_selected_questions)
